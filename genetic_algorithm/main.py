...
def tournament_selection(group_size, population): #TODO jakies zabezpieczenie zeby nie bylo jakis ulamków czy cos(?)
...
def single_point_crossing(parent_a, parent_b, crossover_probability):
    if random.random() < crossover_probability:
        ...
        crossing_point = random.randint(1,
                                        7)  # wykluczamy wybranie wszystkich genów z 1 rodzica, 7 bo i tak jest to excluded w rangu
...
def two_point_crossing(parent_a, parent_b, crossover_probability):
    if random.random() < crossover_probability:
        ...
        while True:
            ...
            if number1 != number2 and abs(
                    number1 - number2) > 2:  # liczby nie mogą być identyczne ani 2 miejsca od siebie (bo excluded) weic de facto 1 miejsce
                break
...
def individual_mutation(individual):
    random_bit = random.randint(0, len(individual))
